...
export SERVICE_IP_RANGE=10.30.0.0/22
...
export K8S_SERVICE_IP=10.30.0.1
...
export DNS_SERVICE_IP=10.30.0.10
...
export NODE_GATEWAY=10.10.10.1

export NODE_START_IP=10.10.10.10
...
function createTemplates {
  ...
  for ip in $(seq $(($d)) $(($NUMBER_NODES+$d-1)));do
    ...
    while IFS= read -r line ; do
      ...
      echo -e "$line"
    done <<< "$TEMPLATE" > user-data-$NODE_IP
...
mkdir -p ssl
./cert-gen/init-ca.sh ./ssl
./cert-gen/init-ssl.sh ./ssl apiserver kube-controller $(buildSANIP)
./cert-gen/init-ssl.sh ./ssl worker kube-worker
